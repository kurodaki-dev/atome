...
CPU uniquement (aucun backend GPU n'existe dans TON618), tenseurs 1D/2D seulement, pas de diffusion (broadcasting) façon NumPy au-delà d'un scalaire, et c'est lent (boucles TON618 interprétées, pas de vectorisation). Voir [`DOCUMENTATION.fr.md`](DOCUMENTATION.fr.md) pour le détail complet et pourquoi.
